        ...
        print(
            " - ".join(
                [
                    ...
                    f"failed to resample: {failed_to_resize_ratio:.3f}",
                    f"audio per sec: {audio_per_sec:.0f}",
                    f"count: {count}",
                ]
            )
        )
...
class LoggerProcess(multiprocessing.context.SpawnProcess):
    ...
    def __init__(self, output_folder, config_parameters, log_interval=5):
        super().__init__()
    ...
    def run(self):
        ...
        self.total_speed_logger = SpeedLogger(
            prefix="total", enable_wandb=self.enable_wandb)
