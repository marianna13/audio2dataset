""""writer module handle writing the audio to disk"""
...
class WebDatasetSampleWriter:
    ...
    def write(self, audio_array, key, caption, meta):
        """write sample to tars"""
        if audio_array is not None:
            sample = {"__key__": key, self.encode_format: audio_array}
            if self.save_caption:
                sample["txt"] = str(caption) if caption is not None else ""
...
class FilesSampleWriter:
    ...
    def write(self, audio_array, key, caption, meta):
        ...
        if audio_array is not None:
            ...
            sf.write(filename, data=audio_array.T, samplerate=self.sample_rate)
