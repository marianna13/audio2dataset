...
def download_audio_with_retry(row, retries):
    for _ in range(retries + 1):
        key, audio_stream, err = download_audio(row)
        if audio_stream is not None:
...
class Downloader:
    ...
    def resample(self, audio_stream, target_sr):
        try:
            ...
            return y, sr, target_sr, duration, orig_duration, None
    ...
    def download_shard(
        ...
        schema = (
            ...
            .append(pa.field("sample_rate", pa.int32()))
            .append(pa.field("duration", pa.int32()))
            .append(pa.field("original_sample_rate", pa.int32()))
            .append(pa.field("original_duration", pa.int32()))
        )
        ...
        with ThreadPool(self.thread_count) as thread_pool:
            for key, audio_stream, error_message in thread_pool.imap_unordered(
                lambda x: download_audio_with_retry(
                    x, retries=self.retries),
                loader,
            ):
                try:
                    ...
                    ) = self.resample(audio_stream, self.sample_rate)
                    if error_message is not None:
                        failed_to_resample += 1
                        status = "failed_to_resample"
                        status_dict.increment(error_message)
                        meta["status"] = status
                    ...
                    if self.extract_exif:
                        try:
                            ...
                            exif = json.dumps(
                                {
                                    k: str(v).strip()
                                    for k, v in exifread.process_file(audio_stream, details=False).items()
                                    if v is not None
                                }
                            )
                    ...
                    if self.compute_md5:
                        audio_stream.seek(0)
                        meta["md5"] = hashlib.md5(
                            audio_stream.read()).hexdigest()

                    meta["status"] = status
                    meta["sample_rate"] = sample_rate
                    meta["original_sample_rate"] = orig_sample_rate
                    meta["duration"] = duration
                    meta["original_duration"] = orig_duration
                    audio_stream.close()
                    del audio_stream
